...
class Overview {
  ...
  renderPlanets(minLevel,planetType,middle,MyPlanets) {
    ...
    for (const [idx, p] of sortedPlanets.entries()) {
      ...
      const silverPercent = Math.round(100/(planet.silverCap/Math.round(getSilver(planet))));
      const fullSilverDate = new Date(df.getSilverCurveAtPercent(planet,99) * 1000).toString() ;
      const fullSilverTime = [fullSilverDate.substr(4,6),fullSilverDate.substr(16,8)];

      const energyPercent = Math.round(100/(planet.energyCap/Math.round(planet.energy)));
      const fullEnergyDate = new Date(df.getEnergyCurveAtPercent(planet,99) * 1000).toString();
      const fullEnergyTime = [fullEnergyDate.substr(4,6),fullEnergyDate.substr(16,8)];

      row.innerHTML = `<td>${idx + 1}.</td><td>${getPlanetName(planet)}</td><td style="text-align: center">${planet.planetLevel}</td><td>${getSilver(planet)}/${planet.silverCap}=${silverPercent}%</td><td>${fullSilverTime}</td><td>${Math.round(planet.energy)}/${planet.energyCap}=${energyPercent}%</td><td>${fullEnergyTime}</td>`
 ...
 unconfirmedButton.addEventListener('click', () => {
    ...
    function updateTx(){
    dynamicLabel.innerText = 'Moves :  ' + df.getUnconfirmedMoves().length + '  Upgrades :  ' +     df.getUnconfirmedUpgrades().length + '  Wormhole :  ' + df.getUnconfirmedWormholeActivations().length;
    }
