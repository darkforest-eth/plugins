//added isPlayable function to include L3+ planets
...
class Plugin {
  constructor() {
    ...
    msg.innerText = 'Click on the map to pin selection. Also takes L3+ planets';
  ...
  chunkPlayable(chunk) {
    ...
    let filtered =[];
    if(chunk.planetLocations.length >0)  {
         filtered =  chunk.planetLocations
                    .filter ((p) => df.getPlanetWithId(p.hash).planetLevel >=3);
    }

   if (filtered.length >0)  isPlayable = true;
