...
// 
//  Map Filter Export
//  Only export the 16x16 pixel blocks that have filtered planets.
//  
...
//           
//  notice 2: When export the map which has a lot of 16x16 pixel blocks,
...
function mapFilterExport() {
  ...
  let onDownload = async () => { 
    let newInfo = html`<div style=${{color:'#CCFFFF'}}>Begin To Download</div>`;
    setInfo(newInfo);
  ...
  return html`<div  style=${divStyle} >
    ...
    <button style=${buttonStyle} onClick=${onDownload}> Download Map </button>
    ${' '}
