...
(function (PlanetType2) {
    ...
    PlanetType2[PlanetType2["NOT_READY_TO_PROSPECTED"] = 6] = "NOT_READY_TO_PROSPECTED";
    PlanetType2[PlanetType2["FOUND"] = 7] = "FOUND";
})(PlanetType2 || (PlanetType2 = {}));
const PlanetType2Str = ["!RUINS", "wait P", "in P", "wait F", "in F", "expire", "not full", "found"];
...
const checkPlanetType = (planet, currentBlockNumber /*: number*/) => {
    ...
    if (planet.prospectedBlockNumber === undefined) {
        if (planet.energy < 0.95 * planet.energyCap) {
            return PlanetType2.NOT_READY_TO_PROSPECTED;
        }
...
function AppUI(props) { // refreshPlanetStates, doProspectAndFind, chainBlockNumber, planets
    ...
    <div>
        <div style=${{ display: 'flex' }}>
            ...
            <span style=${{ flex: 1 }}></span>
            <span style=${Spacing}>Cur Blk: ${props.chainBlockNumber < 1 ? "Loading" : props.chainBlockNumber}</span>
        </div>
        <div style=${{ display: 'flex' }}>
            ${numsHTML}
        </div>
...
class Plugin {
    ...
    constructor() {
        this._chainBlockNumber_Subscriber = ui.getEthConnection().blockNumber$.subscribe(x => {
            let t = this.chainBlockNumber;
            this.chainBlockNumber = x;
            if (t < 1) {
                this.refreshPlanetStates();
            }
        });
        this.container = null;
